$(document).ready(function(){
		...
		$("[name='poll_bar']").each(
				function(i){
				  //get poll value
					var bar_width = ($("[name='poll_val']").eq(i).text());
					bar_width = bar_width - 25;
					...
					if(bar_width_rule >  10 & bar_width_rule < 25 ){$("[name='poll_bar']").eq(i).addClass("btn btn-sm btn-warning")}
					if(bar_width_rule >= 25){$("[name='poll_bar']").eq(i).addClass("btn btn-sm btn-danger")}

					//Hide dril down divs
					$("#" + ($("[name='tweet']").eq(i).text()) ).hide();
	  });

	  //On click main menu bar show its particular detail div.
	  $("[name='tweet']").click(function()
	  {
	     //Hide all
